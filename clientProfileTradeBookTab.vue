  ...
  <div class='ria-client-profile-ledger-tab-main-wrapper'>
    ...
    <div class="client-profile-table-wrapper">
        <div class='client-profile-download-xls-link'>
         ...
         <a @click="onClickLDownloadTradeBook" :href='downloadLink' download
          class="download-xls-link-text">Download.xls</a>
        </div>
        <div class="ledger-table-main-wrapper">
         <a-table
            :columns='tradeBookColumnData'
            :data-source='tradeBookData'
            :pagination='false'>
...
export default {
  ...
  setup(props) {
    ...
    const tradeBookData = reactive([]);
    const showViewMore = ref(true);
    const staticData = reactive([]);
    const fetchClienLedgerTradeBk = async () => {
      const emptyArray = [];
      await services
        .fetchClientTradeBook(clientId)
        .then((response) => {
          const { data } = response;
      ...
      Object.assign(tradeBookData, emptyArray.slice(0, 10));
    };
    const onClickViewMore = () => {
      Object.assign(tradeBookData, staticData);
      showViewMore.value = false;
    };
    const downloadLink = ref('');
    const onClickLDownloadTradeBook = async () => {
      await services
        .downloadClientTradeBook(clientId)
        .then((response) => {
          const { data } = response;
          console.log('data :>> ', data);
    ...
    onMounted(() => {
      window.scrollTo(0, 0);
      fetchClienLedgerTradeBk(clientId);
    });
    ...
    const tradeBookColumnData = reactive([
      {
        title: 'Symbol',
        dataIndex: 'symbol',
        key: 'symbol',
