  ...
  <div class='ria-client-profile-ledger-tab-main-wrapper'>
    ...
    <div class="client-profile-table-wrapper">
        <div class='client-profile-download-xls-link'>
         ...
         <a @click="onClickLDownloadDpTrans" :href='downloadLink' download
         class="download-xls-link-text">Download.xls</a>
        </div>
        <div class="ledger-table-main-wrapper">
         <a-table
            :columns='dpTransactionColumnData'
            :data-source='dpTransactionData'
            :pagination='false'>
...
export default {
  ...
  setup(props) {
    ...
    const dpTransactionData = reactive([]);
    const staticData = reactive([]);
    const showViewMore = ref(true);
    const fetchDpTransactions = async () => {
      const emptyArray = [];
      await services
        .fetchClientDpTransactions(clientId)
        .then((response) => {
          const { data } = response;
      ...
      Object.assign(dpTransactionData, emptyArray.slice(0, 10));
    };
    const onClickViewMore = () => {
      Object.assign(dpTransactionData, staticData);
      showViewMore.value = false;
    };
    const downloadLink = ref('');
    const onClickLDownloadDpTrans = async () => {
      await services
        .downloadClientDpTransactions(clientId)
        .then((response) => {
          const { data } = response;
          console.log('data :>> ', data);
    ...
    const dpTransactionColumnData = reactive([
      {
        title: 'Symbol',
        dataIndex: 'symbol',
        key: 'symbol',
    ...
    return {
      ...
      segment,
      tags,
      dpTransactionColumnData,
      dpTransactionData,
      clientId,
      onClickLDownloadDpTrans,
      downloadLink,
      onClickViewMore,
      showViewMore,
    };
