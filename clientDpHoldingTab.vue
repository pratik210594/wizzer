  ...
  <div class='ria-client-profile-ledger-tab-main-wrapper'>
    ...
    <div class="client-profile-table-wrapper">
        <div class='client-profile-download-xls-link'>
         ...
         <a @click="onClickLDownloadDpHolding" :href='downloadLink' download
          class="download-xls-link-text">Download.xls</a>
        </div>
        <div class="ledger-table-main-wrapper">
         <a-table
            :columns='dpHoldingColumnData'
            :data-source='dpHoldingData'
            :pagination='false'>
...
export default {
  ...
  setup(props) {
    ...
    const dpHoldingData = reactive([]);
    const staticData = reactive([]);
    const showViewMore = ref(true);
    const fetchClientDp = async () => {
      const emptyArray = [];
      await services
        .fetchClientDpHolding(clientId)
        .then((response) => {
          const { data } = response;
      ...
      Object.assign(dpHoldingData, emptyArray.slice(0, 10));
    };
    const onClickViewMore = () => {
      Object.assign(dpHoldingData, staticData);
      showViewMore.value = false;
    };
    const downloadLink = ref('');
    const onClickLDownloadDpHolding = async () => {
      await services
        .downloadClientDpHolding(clientId)
        .then((response) => {
          const { data } = response;
          console.log('data :>> ', data);
    ...
    const dpHoldingColumnData = reactive([
      {
        title: 'Symbol',
        dataIndex: 'symbol',
        key: 'symbol',
